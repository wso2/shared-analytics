...
var div = "#chartErrorCodeDistribution";
...
function fetch() {
    ...
    var newTo;
    var tomorrow;
    console.log("ErrorCodeDistribution");
    var diffDays = daysBetween(new Date(from), new Date(to));
    if(diffDays>90){
        timeFrame = "monthly";
        queryInfo = {
            tableName: "LOGANALYZER_ERROR_CODE_DISTRIBUTION_MONTHLY",
            searchParams: {
        ...
        timeFrame = "weekly";
        queryInfo = {
            tableName: "LOGANALYZER_ERROR_CODE_DISTRIBUTION_WEEKLY",
            searchParams: {
                query: "_timestamp: [" + from + " TO " + to + "]",
                start : 0, //starting index of the matching record set
                count : 100, //page size for pagination
        ...
        timeFrame = "daily";
        queryInfo = {
            tableName: "LOGANALYZER_ERROR_CODE_DISTRIBUTION_DAILY",
            searchParams: {
                query: "_timestamp: [" + from + " TO " + to + "]",
                start : 0, //starting index of the matching record set
                count : 100, //page size for pagination
        ...
        if (d["status"] === "success") {
            ...
            if(timeFrame==="daily"){
                ...
                for (var i =0; i < receivedData.length ;i++){
                    ...
                    dataM.push([tempDay.toDateString(),receivedData[i].values.ErrorCount,receivedData[i].values.ErrorCode]);
                }
                ...
                for (var i =0; i < receivedData.length ;i++){
                    var tempDay = new Date(receivedData[i].timestamp);
                    dataM.push([mS[tempDay.getMonth()]+" - "+tempDay.getFullYear(),receivedData[i].values.ErrorCount,receivedData[i].values.ErrorCode]);
                }
                ...
                for (var i =0; i < receivedData.length ;i++){
                    var tempDay = new Date(receivedData[i].timestamp);
                    dataM.push(["W"+receivedData[i].values.week+" "+mS[tempDay.getMonth()]+" - "+tempDay.getFullYear(),receivedData[i].values.ErrorCount,receivedData[i].values.ErrorCode]);
                }
            }
            drawChartByClass();
        }
...
function drawChartByClass() {
    $("#chartErrorCodeDistribution").empty();
    var configChart = {
        type: "bar",
        x : "day",
        ...
        tooltip: {"enabled":true, "color":"#e5f2ff", "type":"symbol", "content":["ErrorCode","count","day"], "label":true}
    };
